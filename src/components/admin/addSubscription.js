...
import validateInput, { validatePriceMonth } from "../../common/validations/validationPlanAdd";
...
class addSubscription extends Component {
  ...
  getPlanDetails = () => {
    ...
    ELPViewApiService("superadmin_getplandetails", data).then((result) => {
      ...
      if (result && result.status === 200) {
        planObj = result && result.data && result.data.data ? result.data.data[0] : [];
        planObj.plan_cat_name &&
          planObj.plan_cat_name.forEach((item) => {
            cats.push(item.puc_cat_id);
          });
        planObj.pl_price_month = planObj.plan_month_price ;

        for (let index = 0; index < planObj.pl_price_month.length -1; index++) {
          errors.pl_price_month.push(kp);
        }
      }
      proffCat.map((item) => {
  ...
  handleSubmit = () => {
    ...
    this.setState(
      ...
      () => {
        data = {
          ...
          pl_price_month: this.state.planObj.pl_price_month
        };
        if (
          +data.plan_type == 1 ||
          data.plan_type == "1" ||
    ...
    setTimeout(() => {
      const validate = validatePriceMonth(this.state.planObj.pl_price_month);
      console.log("Error", this.state.planObj.pl_price_month);
      if (this.isValid(data) && validate.length === 0) {
        ...
        errors.pl_price_month = validate.length === 0 ? errors.pl_price_month : validate;
        this.setState({
          errors: errors,
          showLoader: false,
        });
  ...
  handleSlider = (event, index) => {
    ...
    let planObj = this.state.planObj;
    planObj.pl_price_month[index].pp_max_range_month = `${event}`;
    this.setState(
      {
        sliderChange: event,
        planObj
      ...
      () => {
        console.log("SLIDER CHANGE INDEX", index, this.state.planObj);
      }
    );
  };
  ...
  handlePriceMonth = (event, index) => {
    const { name, value } = event.target;
    let planObj = this.state.planObj;
    planObj.pl_price_month[index][name] = value;
    planObj.pl_price_month[index].pp_min_range_month = "1";
    this.setState({ planObj }, () => console.log("CHANGE PRICE MONTH ", planObj));
  }
  ...
  removeADDEDMonths = (index) => {
    let planObj = this.state.planObj;
    planObj.pl_price_month.splice(index, 1);
    const errorpl_price_month = this.state.errors.pl_price_month;
    errorpl_price_month.splice(index, 1);
    this.setState(
      { planObj, errorpl_price_month },
      () => console.log("Removed MONTH", planObj, errorpl_price_month)
    );
  }
                ...
                <Col md={8} lg={9} className="pl-1">
                  <div className="corporateMember subscriptionplan">
                    ...
                    <Form>
                      ...
                      {planObj.pl_price_month && planObj.pl_price_month.map((plan_pr_month, index) => {
                        return (
                          <>
                            <Form.Group className="mb-4">
                              <Form.Label className="fs20 fw600 col14">
                              ...
                              <div className="slider">
                                <Slider
                                  ...
                                  max={12}
                                  // step={3}
                                  value={plan_pr_month.pp_max_range_month}
                            ...
                            <Row>
                              ...
                              <Col md={6}>
                                <Form.Group className="mt-1 mb-4">
                                  ...
                                  <Form.Control
                                    ...
                                    className="inputTyp2"
                                    name="pp_discount"
                                    value={plan_pr_month.pp_discount}
                                    onChange={(e) => {
                                      this.handlePriceMonth(e, index)
                                    }
                                    }
                              ...
                              {planObj.pl_price_month.length > 1 ? (
                                <Col md={1}>
                                  <Image
                                    src={Deleteicon}
                                    alt=""
